...
export default function TableCupomFiscal() {
  ...
  const openApproveModal = (row: NfceRow) => {
    const currentItems = Array.isArray(row.itemsJson?.items) ? row.itemsJson.items : [];
    const mappedItems = currentItems
      .map((item) => ({
        description: String(item?.description ?? ""),
        quantity: item?.quantity == null ? "" : String(item.quantity),
        unitValue: item?.unitValue == null ? "" : String(item.unitValue),
        totalItemValue: item?.totalItemValue == null ? "" : String(item.totalItemValue),
      }))
      .filter((item) => item.description || item.quantity || item.unitValue || item.totalItemValue);
  ...
  const handleConfirmApprove = async () => {
    ...
    const totalAmountReais = normalizedItems.reduce((acc, i) => acc + (i.totalItemValue ?? 0), 0);
  ...
  const addApproveItem = () => {
    ...
    setTimeout(() => {
      approveItemsScrollRef.current?.scrollTo({ top: approveItemsScrollRef.current.scrollHeight, behavior: "smooth" });
    }, 0);
  };
              ...
              {rows.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-3">
                    <button
                      onClick={() => copyUserUuid(row)}
                      disabled={!row.userUuid}
                      className="text-purple-600 underline hover:text-purple-800 disabled:text-gray-400 disabled:no-underline"
                    >
                      {row.userName}
                    </button>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={!row.url}
                        className="px-2 py-1 text-xs rounded bg-gray-100 text-gray-600 hover:bg-gray-200 disabled:opacity-40"
                        title="Abrir URL"
                      >
                        🔗
                      </button>
                      <button
                        ...
                        disabled={submitting}
                        className="px-2 py-1 text-xs rounded bg-purple-100 text-purple-700 hover:bg-purple-200 disabled:opacity-40"
                        title="Aprovar"
                      >
                        ✓ Aprovar
                      </button>
                      <button
                        ...
                        disabled={submitting}
                        className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 disabled:opacity-40"
                        title="Reprovar"
                      >
                        ✕ Reprovar
                      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col p-6 gap-4">
            ...
            <div className="space-y-3 overflow-y-auto flex-1">
              <input
                readOnly
                value={selected.chaveAcesso}
                className="w-full px-3 py-2 border rounded-lg bg-gray-50 text-sm text-gray-500"
                placeholder="Chave de acesso"
              />
              <input
                value={approveCnpj}
                onChange={(e) => setApproveCnpj(e.target.value)}
                className="w-full px-3 py-2 border rounded-lg text-sm"
                placeholder="CNPJ (somente números)"
              />
              <input
                value={approveEstablishment}
                onChange={(e) => setApproveEstablishment(e.target.value)}
                className="w-full px-3 py-2 border rounded-lg text-sm"
              ...
              <div className="flex gap-2">
                <input
                  type="date"
                  value={approveIssuedDate}
                  onChange={(e) => setApproveIssuedDate(e.target.value)}
                  className="flex-[2] px-3 py-2 border rounded-lg text-sm"
                />
                <input
                  type="time"
                  value={approveIssuedTime}
                  onChange={(e) => setApproveIssuedTime(e.target.value)}
              ...
              <div className="flex justify-between items-center">
                ...
                <button onClick={addApproveItem} disabled={submitting} className="text-xs text-purple-600 font-bold hover:underline">
                  + Adicionar item
                </button>
              </div>

              <div ref={approveItemsScrollRef} className="max-h-[30vh] overflow-y-auto space-y-2">
                {approveItems.map((item, idx) => (
                  <div key={idx} className="border rounded-lg p-3 space-y-2">
                    <input
                      value={item.description}
                      onChange={(e) => updateApproveItem(idx, "description", e.target.value)}
              ...
              <p className="text-sm text-gray-500">Total: R$ {approveItemsTotal.toFixed(2)}</p>
            </div>

            <div className="flex justify-end gap-3 pt-2 border-t">
              <button onClick={closeApproveModal} disabled={submitting} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                Cancelar
              </button>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={rejectReasonCode === "INVALID_URL"}
                  onChange={() => setRejectReasonCode((p) => p === "INVALID_URL" ? "" : "INVALID_URL")}
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={rejectReasonCode === "SITE_UNREADABLE"}
                  onChange={() => setRejectReasonCode((p) => p === "SITE_UNREADABLE" ? "" : "SITE_UNREADABLE")}
                  className="w-4 h-4 text-purple-600"
                />
                Site não disponível
              </label>
            ...
            <div className="flex justify-end gap-3 pt-2 border-t">
              <button onClick={closeRejectModal} disabled={submitting} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                Cancelar
              </button>
